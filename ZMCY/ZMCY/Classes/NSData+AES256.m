...
#define PASSWORD @"81dc9bdb52d04dc20036dbd8313ed055"
...
+ (NSString *)AES256DecryptWithCiphertext:(NSString *)ciphertexts{
    ...
    cipherData=[NSData hex2byte:ciphertexts];
	// 'key' should be 32 bytes for AES256, will be null-padded otherwise
	char keyPtr[kCCKeySizeAES256+1]; // room for terminator (unused)
	bzero(keyPtr, sizeof(keyPtr)); // fill with zeroes (for padding)
...
+ (NSData*)hex2byte:(NSString*)strhex {
    if (strhex == nil) {
        return nil;
    }
    long l = [strhex length];
    if (l % 2 == 1) {
        return nil;
    }
    Byte b[l/2];
    //获取指针的个数
    for (int i=0;i!=l/2;i++) {
        NSString *str=[strhex substringWithRange:NSMakeRange(i*2,2)];
        //十六进度字符串转为整型
