...
- (id)init
{
    ...
    if(self){
        ...
        [self.button1 setTitle:@"商城" forState:UIControlStateNormal];
        ...
        [self.button2 setTitle:@"展会" forState:UIControlStateNormal];
...
- (void)change:(UIButton*)sender
{
    ...
    if(sender.tag==1){
        ...
        url=[NSString stringWithFormat:@"%@/156/ios_shop_list.html",HTTP_URL];
    }else if(sender.tag==2){
        [self.button1 setBackgroundColor:[UIColor clearColor]];
        [self.button2 setBackgroundColor:COLOR078187];
        url=[NSString stringWithFormat:@"%@/60/ios_zhanhui.html",HTTP_URL];
    }
